...
import Foundation
public protocol StateMLogDelegete: class {
    func log(msg: String)
}
open class StateMachine<State: Hashable, Event: Hashable> {
    public weak var logDelegate: StateMLogDelegete?
...
//            self.log(message: "Processed pre condition for event '\(event)' from '\(transition.source)' to '\(transition.destination)'")
            ...
            self.log(message: "\(event):\(previousState) --> \(transition.destination)")
            self.callbackQueue.async {
